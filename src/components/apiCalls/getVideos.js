...
async function getvideos(queryType ,key) {
    ...
    if(queryType == "search") dynamicUrl = `search/?q=${key}hl=hi&gl=IN`;
    else if(queryType == "related") dynamicUrl = `video/related-contents/?id=${key}&hl=hi&gl=IN`
    else dynamicUrl = `home/?hl=hi&gl=IN`;
